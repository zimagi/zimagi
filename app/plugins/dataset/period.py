...
class Provider(BaseProvider("dataset", "period")):
    ...
    def initialize_dataset(self, config):
        ...
        if config.start_time:
            if config.end_time:
                config.units = time_processor.distance(
                    config.start_time, config.end_time, unit_type=config.unit_type, include_direction=True
                )
            elif not config.units:
                config.units = time_processor.distance(
                    config.start_time, time_processor.now, unit_type=config.unit_type, include_direction=True
                )
    ...
    def initialize_query(self, config):
        ...
        if config.start_time:
            ...
            if config.units:
                times = [
                    config.start_time,
                    time_processor.shift(config.start_time, config.units, unit_type=config.unit_type, to_string=False),
                ]
            else:
                times = [config.start_time, time_processor.now]

            config.filters[f"{config.index_field}__range"] = sorted(times)
    ...
    def finalize_query(self, query):
        ...
        if query.last_known_value and query.start_time:
            ...
            query.filters[f"{query.index_field}__lte"] = start_time
            query.filters.pop(f"{query.index_field}__range", None)

            query.time_index = True
            query.order = f"-{query.index_field}"

            data = concatenate(data, query.get_record(), ffill=query.forward_fill)
    ...
    def finalize_dataset(self, dataset, data):
        ...
        if dataset.period_fields:
            data = dataset.extend(data, dataset.period_fields, remove_fields=dataset.remove_fields)
        return data
