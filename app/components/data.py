...
class ProfileComponent(profile.BaseProfileComponent):
    ...
    def run(self, name, config):
        ...
        if not queries or not isinstance(queries, dict):
            self.command.error(
                f"Data {name} requires 'queries' dictionary (with each element representing a data query to merge)"
            )
        ...
        for query_name, query_info in queries.items():
            for query_field, field_value in query_info.items():
                if query_field == "required":
                    ...
                    config[f"{query_name}:{query_field}"] = field_value

        if required_types:
            config["required_types"] = required_types

        self.exec("data save", dataset_provider_name=provider, dataset_key=name, dataset_fields=config, groups_keys=groups)
