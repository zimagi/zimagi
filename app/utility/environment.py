...
from utility.filesystem import load_yaml, remove_dir, remove_file, save_yaml
...
class MetaEnvironment(type):
    ...
    def get_module_path(self, name=None):
        ...
        return settings.MANAGER.get_lib_directory("modules", name)
    ...
    def save_env_vars(self, name=None):
        ...
        env_name = self.get_active_env() if name is None else name
        variables = {"ZIMAGI_ENVIRONMENT": env_name}
        with self.lock:
            if env_name not in self.data["environments"]:
                raise OSError(f"Environment {env_name} is not defined")

            for field_name, field_value in self.data["environments"][env_name].items():
                variables[f"ZIMAGI_{field_name.upper()}"] = field_value if field_value is not None else ""
    ...
    def get_env_defaults(self):
        return {"repo": settings.DEFAULT_RUNTIME_REPO, "base_image": settings.DEFAULT_RUNTIME_IMAGE}
    ...
    def set_active_env(self, name):
        ...
        with self.lock:
            if name not in self.data["environments"]:
                raise OSError(f"Environment {name} is not defined")
