...
class Git(object):
    ...
    def checkout(self, branch):
        self.repository.checkout(
            self.repository.lookup_branch(branch)
        )
    ...
    def pull(self, remote = DEFAULT_REMOTE, branch = None):
        ...
        remote_reference = self.repository.lookup_reference(
        ...
        if merge_result & pygit2.GIT_MERGE_ANALYSIS_UP_TO_DATE:
            ...
            head_reference = self.repository.lookup_reference(
            ...
            self.repository.create_commit('HEAD',
                self.user, self.user, 'Merge',
                tree,
                [ self.repository.head.target, remote_reference ]
            )
            self.repository.state_cleanup()
            self.repository.update_submodules(init = True)
        else:
            raise GitError("Unable to pull {} changes from remote {}".format(branch, remote))
    ...
    def commit(self, message, *files):
        ...
        self.repository.create_commit(
            "refs/heads/{}".format(self.reference),
            self.user, self.user, message,
            self.repository.index.write_tree(),
            parents
        )
