        ...
        for agent in self.manager.collect_agents():
            if agent.command[: len(agent_path)] == agent_path:
                ...
                if self.agent_count is not None:
                    self.save_instance(
                        self._config, config_name, {"value": self.agent_count, "value_type": "int"}, quiet=True
                    )
                    count = f"{existing_count} -> {self.agent_count}"

                data.append([self.key_color(self.manager._get_agent_name(agent.command)), self.value_color(count)])

        if found:
            self.table(data, "agent-counts")
        else:
            self.warning(f"No agents found matching: {self.manager._get_agent_name(agent_path)}")
            exit(1)
