            ...
            if formatter_match.group(3):
                for parameter in re.split(r"\s*\,\s*", formatter_match.group(3)):
                    ...
                    if len(option_components) == 2:
                        ...
                        raise FormatterParserError(
                            f"All options passed to formatter parser must be key value pairs ({value} given)"
                        )

            return self.command.get_provider("formatter", provider, config).format(initial_value, record)
        return value
