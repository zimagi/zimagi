    ...
    def _get_datetime_filter(self, lookup, value, label, formats):
        ...
        for index, format in enumerate(formats):
            try:
                ...
                value = datetime.datetime.fromtimestamp(time.timestamp(), tz = datetime.timezone.utc)
                time_index = index
                break
        ...
        if not time:
            raise ValueError("{} data {} is not a supported format: {}".format(
                label,
                value,
                ",".join(formats)
            ))
...
class DateSearchFilter(DateTimeFilterMixin, CharFilter):
    ...
    def get_filter_data(self, lookup, value):
        return self._get_datetime_filter(lookup, value, "Date", [
            '%Y-%m-%d',
            '%Y-%m',
            '%Y'
        ])
...
class BaseRelatedFilter(object):
    ...
    def get_queryset(self, request):
        ...
        if queryset is None:
            raise FilterError("Method .get_queryset() for related filter '{}.{}' must return a QuerySet".format(
