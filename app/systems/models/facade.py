...
from .mixins import annotations, fields, filters, query, relations, render, update
...
class ModelFacade(
    ...
    def get_packages(self, exclude=None):
        ...
        packages = [settings.DB_PACKAGE_ALL_NAME, self.name if self.name not in exclude else []]
