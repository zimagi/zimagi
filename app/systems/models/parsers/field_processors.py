...
class FieldProcessorParser(BaseParser):
    ...
    def p_expression_assignment(self, p):
        ...
        if isinstance(p[3], FieldProcessor):
            p[3].name = p[1]
            p[0] = p[3]
            logger.debug(f"name = processor: {p[0]}")
    ...
    def p_processor(self, p):
        ...
        p[0] = FieldProcessor(name=f"{processor_name}({field_name})", provider=processor_name, field=field_name)
        logger.debug(f"processor: {p[0]}")
    ...
    def p_processor_with_args(self, p):
        ...
        p[0] = FieldProcessor(
            name="{}({}{}{})".format(
                processor_name,
                f"{field_name}," if arguments or options else field_name,
