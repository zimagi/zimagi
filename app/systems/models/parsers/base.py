...
class PlyLogger:
    ...
    def warning(self, msg, *args, **kwargs):
        if msg not in ("Token %r defined, but not used", "There are %d unused tokens"):
...
class BaseParser:
    ...
    parser_index = {
        ...
        "p_expression_string": (),
        "p_expression_field": (),
        "p_expression_list": ("p_empty_list", "p_list"),
        "p_expression_dictionary": ("p_empty_dictionary", "p_dictionary"),
        "p_expression_db_function": ("p_db_function", "p_db_function_with_params"),
        "p_expression_paren": (),
        "p_expression_calculation": (),
        "p_empty_list": (),
        "p_list": ("p_list_elements", "p_list_elements_single"),
        "p_list_elements": ("p_list_element",),
        "p_list_elements_single": ("p_list_element",),
        "p_list_element": (),
        "p_empty_dictionary": (),
        "p_dictionary": ("p_dictionary_elements", "p_dictionary_elements_single"),
        "p_dictionary_elements": ("p_dictionary_element",),
        "p_dictionary_elements_single": ("p_dictionary_element",),
        "p_dictionary_element": (),
        "p_db_function": (),
        "p_db_function_with_params": ("p_parameters", "p_parameters_single"),
        "p_parameters": ("p_parameter_argument_data", "p_parameter_argument_field", "p_parameter_option"),
        "p_parameters_single": ("p_parameter_argument_data", "p_parameter_argument_field", "p_parameter_option"),
        "p_parameter_argument_data": ("p_empty_list", "p_list", "p_empty_dictionary", "p_dictionary"),
        "p_parameter_argument_field": ("p_db_function", "p_db_function_with_params"),
        "p_parameter_option": ("p_empty_list", "p_list", "p_empty_dictionary", "p_dictionary"),
    }
    ...
    def __init__(self, facade=None):
        ...
        self.lexer = lex.lex(module=self, optimize=False, debug=False, errorlog=PlyLogger(sys.stderr))
        self.parser = yacc.yacc(
            module=self, start="statement", optimize=False, debug=False, write_tables=False, errorlog=PlyLogger(sys.stderr)
        )
    ...
    def evaluate(self, value):
        logger.debug(f"===== Query filter statement ===== ( {value} )")
        logger.debug(f"  === Active base parsers === {self.base_parsers}")
    ...
    #
    def t_error(self, t):
        raise ParseError(f"Illegal expression in query parser: {t.value}")

    def p_error(self, p):
        raise ParseError(f"Parse error: {p}")
    ...
    def p_statement(self, p):
        ...
        """
        logger.debug(f"=== AST         : {p[1]}")
        p[0] = self.process(p[1])
        logger.debug(f"=== Result      : {p[0]}")

        if self.facade:
            logger.debug(f"=== Annotations : {self.facade.get_annotations(False)}")
...
def p_expression_db_function(self, p):
    ...
    """
    p[0] = p[1]
    logger.debug(f"db_function: {p[0]}")
...
def p_expression_calculation(self, p):
    ...
    logger.debug(f"expression op expression: {p[0]}")
...
def p_list_elements(self, p):
    ...
    """
    p[0] = p[1] + (p[3],)
    logger.debug(f"list_elements: {p[0]}")
...
def p_list_element(self, p):
    ...
    """
    p[0] = p[1]
    logger.debug(f"list element: {p[0]}")
...
def p_empty_dictionary(self, p):
    ...
    """
    p[0] = {}
    logger.debug(f"empty dictionary: {p[0]}")
...
def p_dictionary_elements(self, p):
    ...
    """
    p[0] = p[1] + (p[3],)
    logger.debug(f"dictionary_elements: {p[0]}")
...
def p_dictionary_element(self, p):
    ...
    """
    p[0] = (p[1], p[3])
    logger.debug(f"dictionary element: {p[0]}")
...
def p_db_function(self, p):
    ...
    field_name = ":".join(p[1])
    function = p[1][1]
    annotations = {field_name: [function, F(p[1][0])]}
    if self.facade:
        self.facade.add_annotations(**annotations)

    p[0] = F(field_name)
    logger.debug(f"db_function: {p[0]} {annotations}")
...
def p_db_function_with_params(self, p):
    ...
    annotations = {field_name: [function, *arguments, options]}
...
def p_parameter_argument_data(self, p):
    ...
    """
    p[0] = ("ARG", p[1])
    logger.debug(f"data argument: {p[0]}")
...
def p_parameter_argument_field(self, p):
    ...
    """
    p[0] = ("ARG", F(p[1]))
    logger.debug(f"field argument: {p[0]}")
