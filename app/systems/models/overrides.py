...
def _override_model_new(cls, name, bases, attrs, **kwargs):
    ...
    try:
        ...
        logger.info(f"Converting model {name} to an abstract object because it is not in INSTALLED_APPS")
        return _base_model_new(cls, name, bases, attrs, **kwargs)
    ...
    if not cls._meta.abstract:
        ...
        if related_name:
            ...
            if related_name in class_attributes:
                related_name = f"{related_name}_set"
        ...
        if self.remote_field.related_query_name:
            ...
            if related_query_name in class_attributes:
                related_query_name = f"{related_query_name}_set"
