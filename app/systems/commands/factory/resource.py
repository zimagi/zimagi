...
from .operations import list, get, save, remove, clear
from .helpers import *
...
def ResourceCommandSet(command, parents, base_name, facade_name,
    ...
    if allow_list:
        command['list'] = list.ListCommand(
            parents, base_name, facade_name,
            view_roles = view_roles
        )
    if allow_access:
        command['get'] = get.GetCommand(
            parents, base_name, facade_name,
            view_roles = view_roles
        )
    if allow_update:
    ...
    if allow_remove:
        command['remove'] = remove.RemoveCommand(
            parents, base_name, facade_name,
            edit_roles = edit_roles
        )
        if allow_clear:
            command['clear'] = clear.ClearCommand(
                parents, base_name, facade_name,
                edit_roles = edit_roles
            )
    return command
