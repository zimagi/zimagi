...
class ExecCommand(
    ...
    def parse_worker_task_retries(self):
        self.parse_variable('task_retries', '--retries', int,
    ...
    def submit(self, channel, message, suffix = ''):
        return_channel = "command:submit:{}{}".format(self.log_entry.name, ":{}:{}".format(Time.now_string, suffix) if suffix else '')
    ...
    def _exec_local_header(self, log_key, primary = True, task = False, host = None):
        ...
        if primary and self.display_header() and self.verbosity > 1 and not task:
            if host:
                self.data('> host',
                    "{}:{}".format(host.host, host.command_port) if host.command_port else host.host,
                    'host',
                    log = False
                )
            self.data('> env',
    ...
    def _exec_api_handler(self, log_key):
        raise NotImplementedError("Method _exec_api_handler must be implemented in subclasses of the Executable Command class")
