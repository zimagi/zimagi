...
class KubeBase:
    ...
    def _get_labels(self, name, type):
        ...
        return {
            ...
            "app.kubernetes.io/component": f"{type}-{self.type}",
    ...
    def _get_node_selector(self):
        ...
        if not self._check_development():
            node_selector = {"service": f"worker-{self.type}"}
    ...
    def _get_env(self, env_params=None):
        ...
        env = [
            client.V1EnvVar(
                ...
                value_from=client.V1EnvVarSource(field_ref=client.V1ObjectFieldSelector(field_path="metadata.namespace")),
            ),
            client.V1EnvVar(
                name="KUBERNETES_POD_NAME",
                value_from=client.V1EnvVarSource(field_ref=client.V1ObjectFieldSelector(field_path="metadata.name")),
            ),
            client.V1EnvVar(name="ZIMAGI_WORKER_PROVIDER", value="kubernetes"),
            client.V1EnvVar(name="ZIMAGI_WORKER_TYPE", value=self.type),
        ]
        for env_name, env_value in env_params.items():
            env.append(client.V1EnvVar(name=env_name, value=str(env_value)))
    ...
    def _get_volume_mounts(self):
        ...
        if not self._check_development():
            volumes = [
                client.V1Volume(
                    ...
                    persistent_volume_claim=client.V1PersistentVolumeClaimVolumeSource(
                        claim_name=settings.KUBERNETES_LIB_PVC, read_only=False
                    ),
                )
            ]
            volume_mounts = [
                client.V1VolumeMount(name=settings.KUBERNETES_LIB_PVC, mount_path=settings.ROOT_LIB_DIR, read_only=False)
            ]
        else:
            volumes = [
                client.V1Volume(
            ...
            volume_mounts = [
                client.V1VolumeMount(name="app-source", mount_path=settings.APP_DIR, read_only=False),
                client.V1VolumeMount(name="app-lib", mount_path=settings.ROOT_LIB_DIR, read_only=False),
            ]
        return volumes, volume_mounts

    def _get_metadata_spec(self, labels, name=None):
        if name:
            metadata = client.V1ObjectMeta(namespace=self.cluster.namespace, name=name, labels=labels, annotations={})
        else:
        ...
        return client.V1Container(
            ...
            env_from=[
                client.V1EnvFromSource(
                    config_map_ref=client.V1ConfigMapEnvSource(name=settings.KUBERNETES_GLOBAL_CONFIG, optional=False)
                ),
                client.V1EnvFromSource(
                    config_map_ref=client.V1ConfigMapEnvSource(name=settings.KUBERNETES_WORKER_CONFIG, optional=False)
                ),
                client.V1EnvFromSource(
                    secret_ref=client.V1SecretEnvSource(name=settings.KUBERNETES_GLOBAL_SECRET, optional=False)
                ),
            ],
            resources=client.V1ResourceRequirements(
                requests={"cpu": self.spec.get("kube_cpu_min", "750m"), "memory": self.spec.get("kube_memory_min", "250Mi")},
                limits={"cpu": self.spec.get("kube_cpu_max", "1000m"), "memory": self.spec.get("kube_memory_max", "500Mi")},
            ),
    ...
    def _get_pod_spec(self, name, labels, command, env=None, restart_policy="Never"):
        ...
        return client.V1PodTemplateSpec(
            ...
            spec=client.V1PodSpec(
                ...
                containers=[self._get_container_spec(name, command, env, mounts)],
                volumes=volumes,
            ),
        )
