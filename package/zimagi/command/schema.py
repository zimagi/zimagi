...
class Field:
    def __init__(self, name, type=None, required=False, location=None, schema=None, secret=False, system=False, tags=None):
        self.name = name
        self.type = type
...
class Document(OrderedDict):
    ...
    @property
    def data(self):
        return OrderedDict([(key, value) for key, value in self.items() if not isinstance(value, Link)])

    @property
    def links(self):
        return OrderedDict([(key, value) for key, value in self.items() if isinstance(value, Link)])
...
class Link:
    def __init__(self, url=None, action=None, encoding=None, title=None, description=None, resource=None, fields=None):
        self.url = "" if url is None else url
