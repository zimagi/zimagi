...
from .. import exceptions, parallel, settings, utility
from . import codecs, transports


class Client(client.BaseAPIClient):
    def __init__(
        self, port=settings.DEFAULT_DATA_PORT, verify_cert=settings.DEFAULT_VERIFY_CERT, options_callback=None, **kwargs
    ):
        super().__init__(
            port=port,
            decoders=[
    ...
    def set_scope(self, data_type, values, parents=None):
        ...
        for scope_field, scope_type in scope_fields.items():
            ...
            if scope_field in values:
                ...
                filters[f"{scope_field}__{scope_key}"] = values[scope_field]
            ...
            def processor():
                ...
                if full:
                    ...
                    def replace_path(path):
                        schema["paths"][path] = self._request("GET", schema["paths"][path]["$ref"])
    ...
    def _execute_type_operation(self, method, data_type, op, options):
        ...
        return self._execute(method, f"{data_type}/{op}", options)

    def _execute_key_operation(self, method, data_type, op, key, options):
        if op is None:
            return self._execute(method, f"{data_type}/{key}", options)
        return self._execute(method, f"{data_type}/{key}/{op}", options)

    def _execute_field_operation(self, method, data_type, op, field_name, options):
        if not field_name:
            field_name = self.get_id_field(data_type)

        return self._execute(method, f"{data_type}/{op}/{field_name}", options)
    ...
    def get_by_key(self, data_type, key, **options):
        results = self._execute_type_operation(
            "GET", data_type, None, {self.get_key_field(data_type): key, **self.set_scope(data_type, options)}
        )
        if results.count is None:
            raise exceptions.ResponseError(f"Instance {data_type} {key}: {results}")
        elif results.count == 0:
            raise exceptions.ResponseError(f"Instance {data_type} {key}: not found")
        elif results.count > 1:
            raise exceptions.ResponseError(f"Instance {data_type} {key}: too many found")
    ...
    def download(self, dataset_name):
        return self._execute("GET", f"download/{dataset_name}")
